...
class SegmentationTask(pl.LightningModule):
    ...
    def forward(self, config, input_patch, input_spatch, input_dates, input_mtd):
        # Mus� vracet to sam� co model - tedy oba v�stupy
        logits_utae, logits_unet = self.model(config, input_patch, input_spatch, input_dates, input_mtd)
        return logits_utae, logits_unet
    
    def step(self, batch):
        
        #patch      = batch['patch'][:, :3, :, :]
        num_ch = self.config["num_channels_aerial"]#pomoci if, nebo pres list, tam muzu vybrat ktery kanal
        #patch  = batch['patch'][:, :num_ch, :, :]
        
        if num_ch == 4:
            patch = batch['patch'][:, :4, :, :] #Pro NIR a RGB
        elif num_ch == 1 and self.config.get("use_nir", False):
            #Pokud chceme jen 1 kanal a je to NIR, bereme index 3
            patch = batch['patch'][:, 3:4, :, :] 
        elif num_ch == 1 and self.config.get("use_dsm", False):
            #jen vyskovy model, byl by to index 4
            patch = batch['patch'][:, 4:5, :, :]
        else:
            #Standardni chovani (RGB nebo v�ech 5)
            patch = batch['patch'][:, :num_ch, :, :]
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        num_ch = self.config["num_channels_aerial"]
        #patch_input = batch["patch"][:, :num_ch, :, :]

        if num_ch == 4:
            patch_input = batch['patch'][:, :4, :, :]
        elif num_ch == 1 and self.config.get("use_nir", False):
            # jen 1 kanal a je to NIR, bereme index 3
            patch_input = batch['patch'][:, 3:4, :, :] 
        elif num_ch == 1 and self.config.get("use_dsm", False):
            #jen vyskovy model, byl by to index 4
            patch_input = batch['patch'][:, 4:5, :, :]
        else:
            #Standardni chovani (RGB nebo vsech 5)
            patch_input = batch['patch'][:, :num_ch, :, :]

        _, logits_unet = self.forward(self.config, patch_input, batch['spatch'], batch['dates'], batch["mtd"])
