    ...
    def get_data_paths(config: dict, path_domains: str, paths_data: dict, matching_dict: dict, test_set: bool) -> dict: 
        ...
        for domain in path_domains: 
            for area in os.listdir(Path(paths_data['path_aerial_'+status], domain)): 
                ...
                #preskocim soubor, pokud ma jiny tvar nez potrebuju
                aerial = sorted(
    [
        x for x in list_items(Path(paths_data['path_aerial_' + status]) / domain / Path(area), 'IMG*.tif')
        if x.split('_')[-1][:-4].isdigit()
    ],
    key=lambda x: int(x.split('_')[-1][:-4])
)
#tady jsem menila jeden radek, protoze byl problem s nacitanim dat a verzemi!!
